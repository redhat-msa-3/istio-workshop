...
git clone https://github.com/redhat-developer-demos/istio-tutorial/ /root/projects/istio-tutorial || { echo "Error running git clone"; exit 1; }
...
mvn package -f /root/projects/istio-tutorial/preference/ -DskipTests &> /dev/null || { echo "Error building preferences"; exit 1; }
echo "Building recommendations microservice with Maven"
mvn package -f /root/projects/istio-tutorial/recommendation/ -DskipTests &> /dev/null || { echo "Error building recommendations"; exit 1; }
...
docker build -t example/preference /root/projects/istio-tutorial/preference/ &> /dev/null || { echo "Error building preferences docker image"; exit 1; }
echo "Building recommendations docker image"
docker build -t example/recommendation:v1 /root/projects/istio-tutorial/recommendation/ &> /dev/null || { echo "Error building recommendations docker image"; exit 1; }
...
echo "All 3 microservices (customer, preference, recommendation) have been deployed"
