...
`oc create -f istiofiles/route-rule-recommendations-v2_503.yml -n tutorial`{{execute}}
...
Now check the file [oute-rule-recommendations-v2_retry.yml](https://github.com/redhat-developer-demos/istio-tutorial/blob/master/istiofiles/route-rule-recommendations-v2_retry.yml).
...
To check the old 503 error behaviour, try the microservice several times by typing `while true; do time curl http://customer-tutorial.[[HOST_SUBDOMAIN]]-80-[[KATACODA_HOST]].environments.katacoda.com; sleep .1; done`{{execute}}
